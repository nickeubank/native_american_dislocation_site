...
def plot_points(
    ...
    ax.set_title(
        ...
        + "\n"
        + level.capitalize()
        + f" Legislative Chamber in {map_year-1}",
        fontsize=16,
    )
    ...
    ax.figure.text(
        0.13,
        0.05,
        f"{map_year-1} Montana State House district map. District American Indian (AI) share of Voting Age"
    ...
    ax.figure.savefig(
        f"../../30_paper/maps/"
        f"MT_{level}31_specific_map{map_year}_{state_abbrevs[state_fips]}_census{census_year}_{sample_pct * 100:.0f}pct.pdf"
    )

    ax.figure.savefig(
        f"../../30_paper/maps/"
        f"MT_{level}31_specific_map{map_year}_{state_abbrevs[state_fips]}_census{census_year}_{sample_pct * 100:.0f}pct.png"
    )
